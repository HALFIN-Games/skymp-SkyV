...
export class AuthService extends ClientListener {
  ...
  private tryRequestSkyvCharacters() {
    if (!this.isSkyvJoinUiEnabled()) return;
    const authData = this.sp.storage[authGameDataStorageKey] as AuthGameData | undefined;
    if (!authData?.remote?.session) return;
    if (this.skyvListRequested) return;
    this.skyvListRequested = true;
    const message: CustomPacketMessage = {
      t: MsgType.CustomPacket,
      contentJsonDump: JSON.stringify({
        customPacketType: "skyvCharactersListRequest",
        gameData: { session: authData.remote.session },
      }),
    };
